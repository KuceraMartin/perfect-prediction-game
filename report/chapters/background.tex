...
\subsection{REST API}
REST (representational state transfer) API (Application Programming Interface) is a broad term describing an architecural style where a server and a client communicate on the world wide web.
Communication is usually conducted over the HTTP protocol using GET, POST, PUT, and DELETE methods.
In general, REST allows various formats for serializing data; in this work we use JSON.
