...
\begin{figure}[b]
	...
	\begin{tikzpicture}[line width=0.25pt]
		...
		\draw (-0,0.2) ellipse (1.7 and 0.9);
		\draw (0,0) ellipse (2.5 and 1.7);
		\draw (0,0.4) ellipse (1.5 and 2.1);
		\draw (0.4,0.3) ellipse (2.5 and 3);
		\draw[rounded corners=2ex] (-3.2,3.5) rectangle (3.2,-2.8);
		\draw (4.5,0) ellipse (3.9 and 3);
		\node at (-0.2,0) {sPTBPE};
		\node at (-0.2,0.7) {PTBPE};
		\node at (-0.2,1.35) {PTE};
		\node at (-0.2,2) {sPTBRE};
		\node at (0,2.75) {PTBRE};
		\node at (-2.2,2.8) {IR};
...
First, we can see from our definition of PTBPE and PTBRE that any strategy profile satisfying the strict definition of each equilibrium also satisfies the weak definition of the respective equilibrium.
Thus, we have sPTBPE $\subset$ PTBPE, and sPTBRE $\subset$ sPTBRE.

\begin{remark}
	TODO: inclusions are indeed strict.
\end{remark}

\begin{observation}
	For any game $G = (P, S, \uu)$, if a strategy profile $\vecs \in S$ is a PTBPE, then it is a PTBRE.
	Similarly, if $\vecs$ is a strict PTBPE, then it is a strict PTBRE.
	Thus, we have PTBPE $\subset$ PTBRE, and sPTBPE $\subset$ sPTBRE.
\end{observation}

\begin{proof}
	If $\vecs$ is a PTBPE, it is by definition a perfectly transparent $i$-best profile for every player $i \in P$.
	If $\vecs$ is a perfectly transparent $i$-best profile, then, again by definition, $s_i$ is a perfectly transparent best response to $\vecs_{-i}$.
...
\begin{remark}
	\autoref{tab:ptbpe-not-sub-sptbre} shows that PTBPE $\not\subseteq$ sPTBRE.
\end{remark}
