...
\begin{definition}[Symmetric game in normal form]
	We say a game $G = (P, S, \uu)$ is symmetric if all players have the same sets of strategies: $S_i = S_j\ \forall i, j \in P$, and the payoffs only depend on the set of opponents' strategies (not on who is playing them): $u_i(\vecs) = u_i(s_i', \ppi_{-i})\ \forall i \in P, \vecs \in S, s_i' \in S_i, \ppi_{-i} \text{ is a permutation of } \vecs_{-i}.$
\end{definition}
